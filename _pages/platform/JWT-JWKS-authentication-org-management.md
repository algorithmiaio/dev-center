...
JWTs are submitted using standard bearer tokens and will work for any v1 endpoint in the Algorithmia API. An example cURL command for executing an algorithm using a JWT is as follows. In this code, `CLUSTER_DOMAIN` would be replaced by the unique domain name of the Algorithmia cluster, `GROUP_NAME` would be the name of the account or organization, `ALGO_NAME` would be the name of the algorithm, and `ALGO_VERSION` would be an optional version hash or semantic version (e.g., `0.1.1`).

```sh
curl https://CLUSTER_DOMAIN/v1/algo/GROUP_NAME/ALGO_NAME/ALGO_VERSION \
